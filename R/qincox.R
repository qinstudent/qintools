...
qincox <- function(data=NULL,vars=NULL,time=NULL,status=NULL,reg = NULL,decimal = NULL,P_decimal = NULL,sig_P = NULL,CL = NULL,direction = NULL)
{
  ...
  if (reg %in% c("uni","uni+mul")) {
    reg_table <- data.frame(vars) %>%
      mutate(reg_model = map(data[vars],
      ...
      mutate(b = sprintf(paste0('%0.',decimal,'f'),coef),
  ...
  if ((reg %in% c("mul","uni+mul"))&(!is.null(direction))) {
    ...
    mul_reg_model <- coxph(paste0("Surv(data[,time],data[,status])","~",paste(mul_vars,collapse = "+")) %>%
                  as.formula, data = data)
    step_mul_reg_model_sum <- step(mul_reg_model,direction = direction,trace = FALSE) %>% summary %>% coefficients %>% as.data.frame %>% rownames_to_column
    ...
    while (i <= nrow(ll)) {
      if (!is.na(ll[i,"Pr(>|z|)"])) {
        describe_out[i] <- ifelse(ll[i,"Pr(>|z|)"]<0.05,
                                  paste0(ll[i,"Variables"],"每增加1个单位，研究对象发生",status,"的可能性将增加",
