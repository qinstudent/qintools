#' @title 单+多因素cox回归
#' @description
#' 该函数可用于进行批量单因素cox回归分析以及多因素cox回归分析。
#' @param data *输入数据框
#' @param vars *自变量
#' @param time 若cox回归则需输入生存时间
#' @param status *结局变量
...
qincox <- function(data = NULL, vars = NULL, time = NULL, status = NULL, reg = NULL, decimal = NULL, P_decimal = NULL, sig_P = NULL, CL = NULL, direction = NULL) {
  ...
  if (is.null(status)) stop("Missing status input")
  ...
  # 剔除结局缺失
  if (nrow(subset(data, is.na(data[, status]))) > 0) {
    data <- subset(data, !is.na(data[, status]))
    print(paste0("回归剔除", nrow(subset(data, is.na(data[, status]))), "例结局缺失"))
  }
  #### 2.单因素回归####
  if (reg %in% c("uni", "uni+mul")) {
    reg_table <- data.frame(vars) %>%
      mutate(
        reg_model = map(
          data[vars],
          ~ coxph(Surv(data[, time], data[, status]) ~ .x, data = data)
        ),
      ...
      mutate(
        b = sprintf(paste0("%0.", decimal, "f"), coef),
        z = sprintf(paste0("%0.", decimal, "f"), z),
        P = sprintf(paste0("%0.", P_decimal, "f"), `Pr(>|z|)`),
        P = ifelse(`Pr(>|z|)` < 10^(-P_decimal), paste0("P<", 10^(-P_decimal)), P),
        S.E = sprintf(paste0("%0.", decimal, "f"), `se(coef)`),
        exp_b_lower = exp(coef - qnorm(1 - (1 - CL) / 2) * `se(coef)`) %>% round(digits = decimal),
        ...
        exp_b_upper = exp(coef + qnorm(1 - (1 - CL) / 2) * `se(coef)`) %>% round(digits = decimal),
        ...
        exp_b = round(exp(coef), digits = decimal),
    ...
    # 调整表格
    reg_table %>%
      select(Variables, rowname, b, S.E, z, P, exp_b_CI, `Pr(>|z|)`, exp_b_lower, exp_b_upper) %>%
    ...
    for (i in catrow + seq) {
      reg_table <- reg_table %>%
        add_row(
          Variables = c(reg_table[i, 1], "  "), rowname = c("", levels(data[, reg_table[i, 1]])[1]), b = c("", ""), S.E = c("", ""), z = c("", ""),
    ...
    names(reg_table)[6] <- paste0("HR(", CL * 100, "%CI)")
  }
  #### 3.多因素回归####
  ##### 3.1多因素####
  if ((reg %in% c("mul", "uni+mul")) & is.null(direction)) {
    ...
    mul_reg_model <- coxph(paste0("Surv(data[,time],data[,status])", "~", paste(mul_vars, collapse = "+")) %>%
    ...
    # 调整表格
    mul_reg_table %>%
      select(Variables, levels, b, S.E, z, P, exp_b_CI, `Pr(>|z|)`, exp_b_lower, exp_b_upper) %>%
    ...
    for (i in catrow_mtable + seq2) {
      mul_reg_table <- mul_reg_table %>%
        add_row(
          Variables = mul_reg_table[i, 1], levels = "", b = "", S.E = "", z = "",
          P = "", .before = i
        )
    }
  ...
  if (reg == "uni") {
    ll <- reg_table
    first_out <- paste0("以", time, "为生存时间，", status, "为生存结局，分别以", paste0(vars, collapse = "、"), "为自变量构建批量单因素cox回归模型探索影响因素，结果显示：")
    i <- 1
    describe_out <- vector()
    while (i <= nrow(ll)) {
      if (!is.na(ll[i, "Pr(>|z|)"])) {
        describe_out[i] <- ifelse(ll[i, "Pr(>|z|)"] < 0.05,
          paste0(
            ll[i, "Variables"], "每增加1个单位，研究对象发生", status, "的可能性将增加",
            substr(ll[i, "HR(95%CI)"], 1, regexpr("\\(", ll[i, "HR(95%CI)"]) - 1),
            "倍（HR=", substr(ll[i, "HR(95%CI)"], 1, regexpr("\\(", ll[i, "HR(95%CI)"]) - 1),
            ",", CL * 100, "%CI:", substr(ll[i, "HR(95%CI)"], regexpr("\\(", ll[i, "HR(95%CI)"]) + 1, regexpr("\\)", ll[i, "HR(95%CI)"]) - 1),
            ",p:", ll[i, "P"], "）；"
          ),
          NA
        )
        ...
        for (j in 2:nlevels(data[, ll[i, "Variables"]])) {
          describe_out_vec[j] <- ifelse(ll[i + j, "Pr(>|z|)"] < 0.05,
            paste0(
              ...
              "的研究对象发生", status, "的可能性是",
              ll[i, "Variables"], "为", substr(ll[i + 1, "Variables"], 3, nchar(ll[i + j, "Variables"])),
              "的研究对象的", substr(ll[i + j, "HR(95%CI)"], 1, regexpr("\\(", ll[i + j, "HR(95%CI)"]) - 1),
              "倍（HR=", substr(ll[i + j, "HR(95%CI)"], 1, regexpr("\\(", ll[i + j, "HR(95%CI)"]) - 1),
              ",", CL * 100, "%CI:", substr(ll[i + j, "HR(95%CI)"], regexpr("\\(", ll[i + j, "HR(95%CI)"]) + 1, regexpr("\\)", ll[i + j, "HR(95%CI)"]) - 1),
              ",p:", ll[i + j, "P"], "）；"
            ),
            NA
          )
    ...
    flextable(reg_table, col_keys = names(reg_table)[-(7:9)], defaults = tibble(), theme_fun = theme_booktabs) %>%
      ...
      add_footer_lines(values = "HR: Hazard Ratio, CI: Confidence Interval") %>%
  ...
  if ((reg == "uni+mul") & (is.null(direction))) {
    ...
    while (i <= nrow(ll)) {
      if (!is.na(ll[i, "Pr(>|z|)"])) {
        describe_out[i] <- ifelse(ll[i, "Pr(>|z|)"] < 0.05,
          paste0(
            ll[i, "Variables"], "每增加1个单位，研究对象发生", y, "的可能性将增加",
            substr(ll[i, "HR(95%CI)"], 1, regexpr("\\(", ll[i, "HR(95%CI)"]) - 1),
            "倍（HR=", substr(ll[i, "HR(95%CI)"], 1, regexpr("\\(", ll[i, "HR(95%CI)"]) - 1),
            ",", CL * 100, "%CI:", substr(ll[i, "HR(95%CI)"], regexpr("\\(", ll[i, "HR(95%CI)"]) + 1, regexpr("\\)", ll[i, "HR(95%CI)"]) - 1),
            ",p:", ll[i, "P"], "）；"
    ...
    ll2 <- mul_reg_table
    first_out2 <- paste0("以", time, "为生存时间，", status, "为生存结局，分别以", paste0(vars, collapse = "、"), "为自变量构建批量单因素cox回归模型探索影响因素，结果显示：")
    i <- 1
    describe_out <- vector()
    while (i <= nrow(ll2)) {
      if (!is.na(ll2[i, "Pr(>|z|)"])) {
        describe_out[i] <- ifelse(ll2[i, "Pr(>|z|)"] < 0.05,
          paste0(
            ll2[i, "Variables"], "每增加1个单位，研究对象发生", y, "的可能性将增加",
            substr(ll2[i, "HR(95%CI)"], 1, regexpr("\\(", ll2[i, "HR(95%CI)"]) - 1),
            "倍（HR=", substr(ll2[i, "HR(95%CI)"], 1, regexpr("\\(", ll2[i, "HR(95%CI)"]) - 1),
            ",", CL * 100, "%CI:", substr(ll2[i, "HR(95%CI)"], regexpr("\\(", ll2[i, "HR(95%CI)"]) + 1, regexpr("\\)", ll2[i, "HR(95%CI)"]) - 1),
            ",p:", ll2[i, "P"], "）；"
          ),
          NA
        )
        ...
        for (j in 2:nlevels(data[, ll2[i, "Variables"]])) {
          describe_out_vec[j] <- ifelse(ll2[i + j, "Pr(>|z|)"] < 0.05,
            paste0(
              ...
              "的研究对象的", substr(ll2[i + j, "HR(95%CI)"], 1, regexpr("\\(", ll2[i + j, "HR(95%CI)"]) - 1),
              "倍（HR=", substr(ll2[i + j, "HR(95%CI)"], 1, regexpr("\\(", ll2[i + j, "HR(95%CI)"]) - 1),
              ",", CL * 100, "%CI:", substr(ll2[i + j, "HR(95%CI)"], regexpr("\\(", ll2[i + j, "HR(95%CI)"]) + 1, regexpr("\\)", ll2[i + j, "HR(95%CI)"]) - 1),
              ",p:", ll2[i + j, "P"], "）；"
            ),
            NA
          )
