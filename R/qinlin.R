...
#' @importFrom flextable flextable color padding add_footer_lines set_table_properties body_add_flextable set_flextable_defaults italic bold
qinlin <- function(data=NULL,vars=NULL,y=NULL,reg = NULL,decimal = NULL,P_decimal = NULL,sig_P = NULL,CL = NULL,direction = NULL)
  ...
  reg_table <- data.frame(vars) %>%
    mutate(reg_model = map(data[vars],~ lm(as.formula(paste0(y, "~", ".x")),data = data)),
  ...
  #调整表格
  reg_table %>% select(Variables,rowname,b,S.E,t,P, b_CI,`Pr(>|t|)`,b_lower,b_upper) %>% as.data.frame -> reg_table
  ...
  for (i in catrow+seq) {
    reg_table <- reg_table %>%
      add_row(Variables = c(reg_table[i,1],"  "),rowname = c("",levels(data[,reg_table[i,1]])[1]),b=c("",""),S.E=c("",""),t=c("",""),
  ...
  if ((reg %in% c("mul","uni+mul"))&is.null(direction)) {
    ...
    #调整表格
    mul_reg_table %>% select(Variables,levels,b,S.E,t,P, b_CI,`Pr(>|t|)`,b_lower,b_upper) %>% as.data.frame -> mul_reg_table
    ...
    for (i in catrow_mtable+seq2) {
      mul_reg_table <- mul_reg_table %>%
        add_row(Variables = mul_reg_table[i,1],levels = "",b="",S.E="",t="",
                P="",.before = i)}
  ...
  if ((reg %in% c("mul","uni+mul"))&(!is.null(direction))) {
    ...
    #调整表格
    mul_reg_table2 %>% select(Variables,levels,b,S.E,t,P, b_CI,`Pr(>|t|)`,b_lower,b_upper) %>% as.data.frame -> mul_reg_table2
    ...
    while (i <= nrow(ll)) {
      if (!is.na(ll[i,"Pr(>|t|)"])) {
        describe_out[i] <- ifelse(ll[i,"Pr(>|t|)"]<0.05,
                                  paste0(ll[i,"Variables"],"每增加1个单位，研究对象的",y,"将变化",
    ...
    while (i <= nrow(ll2)) {
      if (!is.na(ll2[i,"Pr(>|t|)"])) {
        describe_out[i] <- ifelse(ll2[i,"Pr(>|t|)"]<0.05,
                                  paste0(ll2[i,"Variables"],"每增加1个单位，研究对象的",y,"将变化",
