...
qinlin <- function(data = NULL, vars = NULL, y = NULL, reg = NULL, decimal = NULL, P_decimal = NULL, sig_P = NULL, CL = NULL, direction = NULL) {
  ...
  if (reg %in% c("uni", "uni+mul")) {
    reg_table <- data.frame(vars) %>%
      mutate(
        reg_model = map(data[vars], ~ lm(as.formula(paste0(y, "~", ".x")), data = data)),
        reg_model_sum = map(reg_model, ~ summary(.x) %>%
          coefficients() %>%
          as.data.frame() %>%
          rownames_to_column()),
        exp_b = map(reg_model, ~ coef(.x) %>% exp())
      ) %>% # 建模
      unnest(c(reg_model_sum, exp_b)) %>% # 解压
      mutate(
        b = sprintf(paste0("%0.", decimal, "f"), Estimate),
        t = sprintf(paste0("%0.", decimal, "f"), `t value`),
        P = sprintf(paste0("%0.", P_decimal, "f"), `Pr(>|t|)`),
        P = ifelse(`Pr(>|t|)` < 10^(-P_decimal), paste0("<", 10^(-P_decimal)), P),
        S.E = sprintf(paste0("%0.", decimal, "f"), `Std. Error`),
        b_lower = (Estimate - qnorm(1 - (1 - CL) / 2) * `Std. Error`) %>% round(digits = decimal),
        b_lower = ifelse(b_lower < 0 & -b_lower < 10^(-decimal), -10^(-decimal),
          ifelse(b_lower > 0 & b_lower < 10^(-decimal), 10^(-decimal), b_lower)
        ),
        b_lower = sprintf(paste0("%0.", decimal, "f"), b_lower),
        b_upper = (Estimate + qnorm(1 - (1 - CL) / 2) * `Std. Error`) %>% round(digits = decimal),
        b_upper = ifelse(b_upper < 0 & -b_upper < 10^(-decimal), -10^(-decimal),
          ifelse(b_upper > 0 & b_upper < 10^(-decimal), 10^(-decimal), b_upper)
        ),
        b_upper = sprintf(paste0("%0.", decimal, "f"), b_upper),
        ...
        b_CI = str_c(b, " (", b_lower, "-", b_upper, ")"),
        Variables = vars,
        across(rowname, ~ str_remove(.x, ".x"))
      ) %>% # 取值
      filter(rowname != "(Intercept)")
    # 筛出显著变量以备多因素回归
    sig_row <- which(reg_table$`Pr(>|t|)` < sig_P)
    sig_vars <- (reg_table[sig_row, 1] %>% unique() %>% as.data.frame())[, 1]
    # 筛出P<0.05变量以备文字描述
    sig_row_description <- which(reg_table$`Pr(>|t|)` < 0.05)
    sig_vars_description <- (reg_table[sig_row_description, 1] %>% unique() %>% as.data.frame())[, 1]
    # 调整表格
    reg_table %>%
      select(Variables, rowname, b, S.E, t, P, b_CI, `Pr(>|t|)`, b_lower, b_upper) %>%
    ...
    for (i in catrow + seq) {
      reg_table <- reg_table %>%
        add_row(
          Variables = c(reg_table[i, 1], "  "), rowname = c("", levels(data[, reg_table[i, 1]])[1]), b = c("", ""), S.E = c("", ""), t = c("", ""),
          P = c("", ""), b_CI = c("", "Reference"), .before = i
        )
    }
    catrow2 <- which(duplicated(reg_table$Variables) & (reg_table$Variables != "  "))
    reg_table[catrow2, "Variables"] <- "  "
    reg_table <- reg_table %>%
      mutate(Variables = paste0(Variables, rowname)) %>%
      select(-rowname)
    reg_table$Variables <- gsub("\t", "  ", reg_table$Variables)
    names(reg_table)[6] <- paste0("β(", CL * 100, "%CI)")
  }
  #### 3.多因素回归####
  ##### 3.1多因素####
  if ((reg %in% c("mul", "uni+mul")) & is.null(direction)) {
    ...
    mul_reg_model_sum <- summary(mul_reg_model) %>%
      coefficients() %>%
      as.data.frame() %>%
      rownames_to_column() %>%
      mutate(exp_b = exp(Estimate))
    ...
    # 筛出P<0.05变量以备文字描述
    mul_sig_row_description <- which(mul_reg_table$`Pr(>|t|)` < 0.05)
    mul_sig_vars_description <- (mul_reg_table[mul_sig_row_description, 1] %>% unique() %>% as.data.frame())[, 1]
    # 调整表格
    mul_reg_table %>%
      select(Variables, levels, b, S.E, t, P, b_CI, `Pr(>|t|)`, b_lower, b_upper) %>%
      as.data.frame() -> mul_reg_table
    ...
    mul_reg_table[catrow_mtable, "b_CI"] <- "Reference"
    seq2 <- 0:(length(catrow_mtable) - 1)
    for (i in catrow_mtable + seq2) {
      mul_reg_table <- mul_reg_table %>%
        add_row(
          Variables = mul_reg_table[i, 1], levels = "", b = "", S.E = "", t = "",
          P = "", .before = i
        )
    }
    ...
    mul_reg_table <- mul_reg_table %>%
      mutate(Variables = paste0(Variables, levels)) %>%
      select(-levels)
    mul_reg_table$Variables <- gsub("\t", "  ", mul_reg_table$Variables)
    names(mul_reg_table)[6] <- paste0("β(", CL * 100, "%CI)")
    mul_reg_table$`b` <- na_if(mul_reg_table$`b`, "NA")
    mul_reg_table$`S.E` <- na_if(mul_reg_table$`S.E`, "NA")
    mul_reg_table$`t` <- na_if(mul_reg_table$`t`, "NA")
    mul_reg_table$`b_lower` <- na_if(mul_reg_table$`b_lower`, "NA") %>% as.numeric()
    mul_reg_table$`b_upper` <- na_if(mul_reg_table$`b_upper`, "NA") %>% as.numeric()
  }
  ##### 3.2多因素逐步####
  if ((reg %in% c("mul", "uni+mul")) & (!is.null(direction))) {
    if (reg == "mul") {
    ...
    mul_reg_model <- lm(as.formula(paste0(y, "~", paste(mul_vars, collapse = "+"))), data = data)
    step_mul_reg_model_sum <- step(mul_reg_model, direction = direction, trace = FALSE) %>%
      summary() %>%
      coefficients() %>%
      as.data.frame() %>%
      rownames_to_column() %>%
    ...
    mul_reg_table2[step_catrow_mtable, "b_CI"] <- "Reference"
    ...
    names(mul_reg_table2)[6] <- paste0("β(", CL * 100, "%CI)")
    mul_reg_table2$`b` <- na_if(mul_reg_table2$`b`, "NA")
    mul_reg_table2$`S.E` <- na_if(mul_reg_table2$`S.E`, "NA")
    mul_reg_table2$`t` <- na_if(mul_reg_table2$`t`, "NA")
    mul_reg_table2$`b_lower` <- na_if(mul_reg_table2$`b_lower`, "NA") %>% as.numeric()
    mul_reg_table2$`b_upper` <- na_if(mul_reg_table2$`b_upper`, "NA") %>% as.numeric()
  }
  #### 4.结果输出####
  ...
  if (reg == "uni") {
    ll <- reg_table
    first_out <- paste0("以", y, "为因变量，分别以", paste0(vars, collapse = "、"), "为自变量构建批量单因素线回归模型探索影响因素，结果显示：")
    i <- 1
    describe_out <- vector()
    while (i <= nrow(ll)) {
      if (!is.na(ll[i, "Pr(>|t|)"])) {
        describe_out[i] <- ifelse(ll[i, "Pr(>|t|)"] < 0.05,
          paste0(
            ll[i, "Variables"], "每增加1个单位，研究对象的", y, "将变化",
            substr(ll[i, "β(95%CI)"], 1, regexpr("\\(", ll[i, "β(95%CI)"]) - 1),
            "个单位（β=", substr(ll[i, "β(95%CI)"], 1, regexpr("\\(", ll[i, "β(95%CI)"]) - 1),
            ",", CL * 100, "%CI:", substr(ll[i, "β(95%CI)"], regexpr("\\(", ll[i, "β(95%CI)"]) + 1, regexpr("\\)", ll[i, "β(95%CI)"]) - 1),
            ",p:", ll[i, "P"], "）；"
          ),
          NA
        )
        ...
        for (j in 2:nlevels(data[, ll[i, "Variables"]])) {
          describe_out_vec[j] <- ifelse(ll[i + j, "Pr(>|t|)"] < 0.05,
            paste0(
              ll[i, "Variables"], "为", substr(ll[i + j, "Variables"], 3, nchar(ll[i + j, "Variables"])),
              "的研究对象的", y, "的均值比",
              ll[i, "Variables"], "为", substr(ll[i + 1, "Variables"], 3, nchar(ll[i + j, "Variables"])),
              "的研究对象变化", substr(ll[i + j, "β(95%CI)"], 1, regexpr("\\(", ll[i + j, "β(95%CI)"]) - 1),
              "个单位（β=", substr(ll[i + j, "β(95%CI)"], 1, regexpr("\\(", ll[i + j, "β(95%CI)"]) - 1),
              ",", CL * 100, "%CI:", substr(ll[i + j, "β(95%CI)"], regexpr("\\(", ll[i + j, "β(95%CI)"]) + 1, regexpr("\\)", ll[i + j, "β(95%CI)"]) - 1),
              ",p:", ll[i + j, "P"], "）；"
            ),
            NA
          )
    ...
    final_out <- ifelse(length(vars[!vars %in% sig_vars_description]) == 0, "",
      paste0(
        paste0(vars[!vars %in% sig_vars_description], collapse = "、"),
        "与结局的发生可能性无关。"
      )
    )
    ...
    flextable(reg_table, col_keys = names(reg_table)[-(7:9)], defaults = tibble(), theme_fun = theme_booktabs) %>%
      ...
      color(j = ~P, i = which(reg_table$`Pr(>|t|)` < 0.05), color = "red", part = "body") %>%
      bold(j = ~P, bold = ifelse(reg_table$`Pr(>|t|)` < 0.05, T, F), part = "body") %>%
      italic(j = ~P, part = "header") %>% # P值斜体
      padding(j = ~Variables, i = which(grepl("  ", reg_table[, "Variables"])), padding.left = 20) %>% # 分组留空
      add_footer_lines(values = "β: Regression Coefficient, CI: Confidence Interval") %>%
    ...
    uni_reg_docx <- read_docx() %>%
      ...
      body_add_par("\t下面是单因素回归分析的表格，表格中Variables为回归模型纳入变量，b为回归系数，S.E为系数标准差，t为统计量", style = "Normal") %>%
  ...
  if ((reg == "mul") & is.null(direction)) {
    ...
    final_out <- ifelse(length(vars[!vars %in% mul_sig_vars_description]) == 0, "",
      paste0(
        paste0(vars[!vars %in% mul_sig_vars_description], collapse = "、"),
        "与结局的发生可能性无关。"
      )
    )
    ...
    flextable(mul_reg_table, col_keys = names(mul_reg_table)[-(7:9)], defaults = tibble(), theme_fun = theme_booktabs) %>%
      align(align = "left", part = "all") %>% # 居左
      color(j = ~P, i = which(mul_reg_table$`Pr(>|t|)` < 0.05), color = "red", part = "body") %>%
      bold(j = ~P, bold = ifelse(mul_reg_table$`Pr(>|t|)` < 0.05, T, F), part = "body") %>%
      italic(j = ~P, part = "header") %>% # P值斜体
    ...
    mul_reg_docx <- read_docx() %>%
      ...
      body_add_par(describe, style = "Normal") %>%
      body_add_par(final_out, style = "Normal") %>%
      body_add_par("\t下面是多因素回归分析的表格，表格中Variables为回归模型纳入变量，b为回归系数，S.E为系数标准差，t为统计量", style = "Normal") %>%
  ...
  if ((reg == "mul") & (!is.null(direction))) {
    ...
    flextable(mul_reg_table2, col_keys = names(mul_reg_table2)[-(7:9)], defaults = tibble(), theme_fun = theme_booktabs) %>%
      align(align = "left", part = "all") %>% # 居左
      color(j = ~P, i = which(mul_reg_table2$`Pr(>|t|)` < 0.05), color = "red", part = "body") %>%
      bold(j = ~P, bold = ifelse(mul_reg_table2$`Pr(>|t|)` < 0.05, T, F), part = "body") %>%
      italic(j = ~P, part = "header") %>% # P值斜体
    ...
    mul_reg_docx2 <- read_docx() %>%
      ...
      body_add_par(describe, style = "Normal") %>%
      body_add_par(final_out, style = "Normal") %>%
      body_add_par("\t下面是多因素逐步回归分析的表格，表格中Variables为回归模型纳入变量，b为回归系数，S.E为系数标准差，t为统计量", style = "Normal") %>%
  ...
  if ((reg == "uni+mul") & (is.null(direction))) {
    ...
    while (i <= nrow(ll2)) {
      if (!is.na(ll2[i, "Pr(>|t|)"])) {
        describe_out[i] <- ifelse(ll2[i, "Pr(>|t|)"] < 0.05,
          paste0(
            ll2[i, "Variables"], "每增加1个单位，研究对象的", y, "将变化",
            substr(ll2[i, "β(95%CI)"], 1, regexpr("\\(", ll2[i, "β(95%CI)"]) - 1),
            "个单位（β=", substr(ll2[i, "β(95%CI)"], 1, regexpr("\\(", ll2[i, "β(95%CI)"]) - 1),
            ",", CL * 100, "%CI:", substr(ll2[i, "β(95%CI)"], regexpr("\\(", ll2[i, "β(95%CI)"]) + 1, regexpr("\\)", ll2[i, "β(95%CI)"]) - 1),
            ",p:", ll2[i, "P"], "）；"
          ),
          NA
        )
        ...
        for (j in 2:nlevels(data[, ll2[i, "Variables"]])) {
          describe_out_vec[j] <- ifelse(ll2[i + j, "Pr(>|t|)"] < 0.05,
            paste0(
              ll2[i, "Variables"], "为", substr(ll2[i + j, "Variables"], 3, nchar(ll2[i + j, "Variables"])),
              "的研究对象的", y, "的均值比",
              ll2[i, "Variables"], "为", substr(ll2[i + 1, "Variables"], 3, nchar(ll2[i + j, "Variables"])),
              "的研究对象变化", substr(ll2[i + j, "β(95%CI)"], 1, regexpr("\\(", ll2[i + j, "β(95%CI)"]) - 1),
              "个单位（β=", substr(ll2[i + j, "β(95%CI)"], 1, regexpr("\\(", ll2[i + j, "β(95%CI)"]) - 1),
              ",", CL * 100, "%CI:", substr(ll2[i + j, "β(95%CI)"], regexpr("\\(", ll2[i + j, "β(95%CI)"]) + 1, regexpr("\\)", ll2[i + j, "β(95%CI)"]) - 1),
              ",p:", ll2[i + j, "P"], "）；"
            ),
            NA
          )
    ...
    final_out2 <- ifelse(length(sig_vars[!sig_vars %in% mul_sig_vars_description]) == 0, "",
      paste0(
        paste0(sig_vars[!sig_vars %in% mul_sig_vars_description], collapse = "、"),
        "与结局的发生可能性无关。"
      )
    )
